...
public class AuxiliarPedidoService {
	...
	public List<PedidoRespostaDTO> mapearEValidarPedidos(Page<Pedido> pedidos, 
			Function<Pedido, PedidoRespostaDTO> mapeador
			)
	{
	    List<PedidoRespostaDTO> pedidosDTO = pedidos.map(mapeador).getContent();
	    validacaoPedidosExistentes(pedidosDTO);
	    return pedidosDTO;
	}
	
	public PedidoTotalEquipamentosPorPeriodoDTO calcularTotalSolicitado(LocalDate inicio, LocalDate fim) {
		long totalSolicitados = contarTotalEquipamentosSolicitadosNoPeriodo(inicio, fim);
		long totalEntregues = contarTotalEquipamentosEntreguesNoPeriodo(inicio, fim);
		long totalPendentes = totalSolicitados - totalEntregues;
		
		return new PedidoTotalEquipamentosPorPeriodoDTO(totalSolicitados, totalEntregues, totalPendentes);
	}
	
	public PedidoTotalEquipamentosPorPeriodoDTO calcularTotalPrevisto(LocalDate inicio, LocalDate fim) {
		long totalPrevistos = contarEquipamentosComPrevisaoNoPeriodo(inicio, fim);
		long totalEntregues = contarEquipamentosEntreguesComPrevisaoNoPeriodo(inicio, fim);
		long totalPendentes = totalPrevistos - totalEntregues;
		
		return new PedidoTotalEquipamentosPorPeriodoDTO(totalPrevistos, totalEntregues, totalPendentes);
	}
	
	public void verificacaoEquipamentosEntreguesDoPedido(EquipamentoPedido equipamentoPedido) {
